...
- **业务 SDK 完整实现（Phase 3）**：rdContext 的 server/theme/http/ssh/sftp 五大分组从空骨架升级为真实 Rust 调用，贯穿权限校验、连接状态检查、SSH 连接失败人话分类（AUTH_FAILED/TIMEOUT/DNS_FAIL/CONN_REFUSED/KEY_INVALID/SESSION_CLOSED/DISCONNECTED/UNKNOWN 8 类，前端 Toast 友好提示 + 原始英文落盘）、审计日志
...
- **SSH API（命令执行 + 失败人话分类）**：复用现有 `ssh_exec` Rust command；新增 cwd 拼接（`cd ${shellEscaped} && cmd`）、30s Promise.race 超时、`classifyConnectFailure` 翻译 Rust 错误堆栈为 FriendlyFailure（headline + suggestion + 原始堆栈 3 字段）；uiStore.pluginDisableAllSsh 全局禁用开关，hostStore.connectionStates 连接状态校验
